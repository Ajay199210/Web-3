...
const btnAjoutTache = document.getElementById('btnAjoutTache') as HTMLButtonElement;
const conteneurTaches = document.getElementById('conteneurTaches') as HTMLDivElement;
const tache = document.getElementById('tache') as HTMLInputElement;
tache.focus();
...
// Créer la tâche
function creationTache(): HTMLDivElement {
    // Création du block principale de la tâche (contenant le texte de la tâche et le bouton de suppression)
    const divTache = document.createElement('div') as HTMLDivElement;
    
    // La valeur entrée par l'usager
    const valeurTache = document.createElement('p') as HTMLParagraphElement;
    valeurTache.innerText = tache.value;
    
    // Bouton de suppression de la tâche
    const btnSupprimerTache = document.createElement('button') as HTMLButtonElement;
